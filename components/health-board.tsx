...
export function HealthBoard() {
  ...
  return (
    <section className="mx-auto grid w-full max-w-6xl gap-8 px-6 pb-14 pt-4 lg:grid-cols-[2fr_1fr]" aria-label="Health signal board">
      <div className="space-y-6 rounded-3xl border border-cyan-900 bg-slate-950/75 p-6 shadow-2xl">
        ...
        <div className="grid gap-4 md:grid-cols-3" aria-label="Global metrics">
        ...
        <div className="rounded-2xl border border-slate-700 bg-slate-900/60 p-4">
          <label className="text-sm text-slate-300" htmlFor="country-filter">Filter Regions</label>
          <input
            id="country-filter"
          ...
          <div className="mt-4 max-h-96 overflow-auto rounded-xl border border-slate-800">
            <table className="w-full text-left text-sm">
              <caption className="sr-only">Regional signal comparison</caption>
              <thead className="bg-slate-900/90 text-slate-300">
                <tr>
                  <th className="px-3 py-2" scope="col">Country</th>
                  <th className="px-3 py-2" scope="col">Active</th>
                  <th className="px-3 py-2" scope="col">Deaths</th>
                  <th className="px-3 py-2" scope="col">Updated</th>
                </tr>
              </thead>
              <tbody>
                {regions.map((row) => (
          ...
          {regions.length === 0 && <p className="mt-3 text-sm text-slate-400">No countries match the current filter.</p>}
        </div>
      </div>

      <aside className="space-y-4 rounded-3xl border border-slate-800 bg-slate-900/75 p-5" aria-label="Source trust envelope">
          ...
          {query.data?.sourceStatus.map((source) => (
            <div key={source.name} className="rounded-lg border border-slate-700 bg-slate-950/70 p-3">
              <p className="flex items-center gap-2 font-medium">
                {source.healthy ? <ShieldCheck size={16} className="text-emerald-300" aria-hidden="true" /> : <Activity size={16} className="text-amber-300" aria-hidden="true" />}
                {source.name}
              </p>
        ...
        <div className="rounded-lg border border-cyan-800/60 bg-cyan-950/40 p-3 text-xs text-cyan-100">
          <p className="flex items-center gap-2"><Globe2 size={14} aria-hidden="true" /> Region comparison is provider-agnostic and schema-validated before render.</p>
        </div>
      </aside>
    </section>
  );
